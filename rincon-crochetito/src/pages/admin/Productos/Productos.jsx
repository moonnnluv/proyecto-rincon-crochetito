...
export default function Productos() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try { const res = await listProductos({ q, size: 50 }); if (alive) setData(res); }
  ...
  const imgOf = (p) => p.imagenUrl || p.imagen || "/img/no_producto.png";
    ...
    <section className="admin-wrap">
      ...
      <div className="table-responsive">
        <table className="table">
          <thead><tr>
            <th>#</th><th>Producto</th><th>Precio</th><th>Stock</th><th>Estado</th><th style={{width:210}}>Acciones</th>
          </tr></thead>
          <tbody>
            ...
            {rows.map((p, i) => (
              <tr key={p.id}>
                ...
                <td className="prod-cell">
                  <img className="thumb" src={imgOf(p)} alt="" />
                  <div>
                ...
                <td>{p.stock ?? 0}</td>
                <td><span className={`badge ${ (p.estado ?? "ACTIVO")==="ACTIVO" ? "ok":"warn"}`}>{p.estado ?? "ACTIVO"}</span></td>
                <td className="actions">
                  <Link className="btn" to={`/admin/productos/${p.id}`}>Editar</Link>
                  <button className="btn" onClick={()=>onToggle(p)}>{(p.estado ?? "ACTIVO")==="ACTIVO" ? "Inactivar":"Activar"}</button>
